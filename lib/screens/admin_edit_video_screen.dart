...
class _AdminEditVideoScreenState extends State<AdminEditVideoScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_titleCtrl.text.trim() != widget.video.title) {
        fields['title'] = _titleCtrl.text.trim();
      }
      if (_descCtrl.text.trim() != widget.video.description) {
        fields['description'] = _descCtrl.text.trim();
      }
      if (_thumbnailCtrl.text.trim() != widget.video.thumbnailUrl) {
        fields['thumbnailUrl'] = _thumbnailCtrl.text.trim();
      }
      if (_videoUrlCtrl.text.trim() != widget.video.videoUrl) {
        fields['videoUrl'] = _videoUrlCtrl.text.trim();
      }
      ...
      await _db.updateVideo(widget.video.id, fields);

      if (mounted) {
        _showSnack('✅ Video updated successfully!');
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          icon: Icon(Icons.expand_more, color: AppTheme.infoColor),
